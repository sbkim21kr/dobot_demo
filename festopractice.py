...
def select_job(y1a, y1c, y18):
    ...
    # Debug readback
    y1a_status = mc.batchread_bitunits(headdevice="Y1A", readsize=1)[0]
    y1c_status = mc.batchread_bitunits(headdevice="Y1C", readsize=1)[0]
    y18_status = mc.batchread_bitunits(headdevice="Y18", readsize=1)[0]
    print(f"[DEBUG] Job switch → Y1A={y1a_status}, Y1C={y1c_status}, Y18={y18_status}")
...
def scan_inputs():
    print("[DEBUG] Scanning X00–X1F inputs...")
    for i in range(0, 32):
        addr = f"X{hex(i)[2:].upper().zfill(2)}"
        try:
            val = mc.batchread_bitunits(headdevice=addr, readsize=1)[0]
            print(f"{addr} = {val}")
        except Exception as e:
        ...
        try:
            if int(mc.batchread_bitunits(headdevice="M100", readsize=1)[0]) == 1:
                ...
                if int(mc.batchread_bitunits(headdevice="M210", readsize=1)[0]) == 1:
                    ...
                    print("[DEBUG] Job2 completed, checking branch condition...")
                    scan_inputs()  # <-- Diagnostic scan of all X inputs

                    try:
                        color_input = mc.batchread_bitunits(headdevice="X06", readsize=1)[0]
                        color_mem   = mc.batchread_bitunits(headdevice="M211", readsize=1)[0]
                        print(f"[DEBUG] After Job2 → X06={color_input}, M211={color_mem}")

                        if int(color_mem) == 1:
                            print("[DEBUG] Branch → Orange path (Job3)")
                            select_job(0,1,1)
                            run_and_store("Job3 Orange","X0E","M212")

                            job3_val = mc.batchread_bitunits(headdevice="M212", readsize=1)[0]
                            print(f"[DEBUG] Job3 result M212={job3_val}")
                            if int(job3_val) == 1:
                            ...
                            print("[DEBUG] Branch → Brown path (Job4)")
                            select_job(1,0,0)  # adjust mapping if needed
                            run_and_store("Job4 Brown","X06","M213")

                            job4_val = mc.batchread_bitunits(headdevice="M213", readsize=1)[0]
                            print(f"[DEBUG] Job4 result M213={job4_val}")
                            if int(job4_val) == 1:
                        ...
                        print("[DEBUG] Exception after Job2 branch:", e)
