...
def perform_handshake_and_read(input_addr):
    # two dummy handshakes
    pulse_bit("Y1E", on_time=0.2, off_time=0.2)
    pulse_bit("Y1E", on_time=0.2, off_time=0.2)
    # third handshake: ON, read, then OFF
        ...
        try:
            ...
            if m100_val == 1:
                ...
                if job2_val == 1:
                    ...
                    job3_val = run_job3()
                    if job3_val == 1:
                        print("Outcome 2: Orange Pass")
                        mc.batchwrite_bitunits(headdevice="M300", values=[1])
                        pulse_bit("Y11")   # green
                    else:
                        print("Outcome 3: Orange Fail")
                    ...
                    job4_val = run_job4()
                    if job4_val == 1:
                        print("Outcome 4: Brown Pass")
                        mc.batchwrite_bitunits(headdevice="M301", values=[1])
                        pulse_bit("Y11")   # green
                    else:
                        print("Outcome 5: Brown Fail")
